...
class DecodingInference:
    ...
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        output = self.tokenizer.decode(
            history_ids[0][input_ids.shape[-1] :], skip_special_tokens=True
        )
        return output

    def generation_using_different_strategies(
        self, prompt: str, strategy_mode: str = "greedy_search"
    ) -> str:
        ...
        strategies: Dict[str, Dict[str, Any]] = {
            ...
            "random": {"temperature": 0.7, "do_sample": True, "top_k": 0, "max_length": 50},
            "top_k": {"temperature": 0.7, "do_sample": True, "top_k": 50, "max_length": 50},
            "top_p": {"do_sample": True, "top_p": 0.9, "top_k": 50, "max_length": 50},
        }

        kwargs = strategies.get(strategy_mode.lower())
        if kwargs is None:
            raise ValueError(
                "Please provide a valid decoding strategy: greedy_search, beam_search, random, top_k, top_p"
            )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        decoding_mode = input(
            "Enter the decoding strategy you want to use (greedy_search, beam_search, random, top_k, top_p): "
        ).lower()
        ...
        response = decoding_inference.generation_using_different_strategies(
            prompt=prompt, strategy_mode=decoding_mode
        )
