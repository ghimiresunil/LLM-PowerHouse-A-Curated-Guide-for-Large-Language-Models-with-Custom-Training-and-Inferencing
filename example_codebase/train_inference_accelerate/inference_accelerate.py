...
def generate_inference(input_text, tokenizer, model, padding, accelerator, is_saved_model=True):
    model_input = tokenizer(
        input_text, max_length=1024, padding=padding, truncation=True, return_tensors="pt"
    )
    if is_saved_model:
        ...
        generated_tokens = accelerator.unwrap_model(model).generate(**model_input, **gen_kwargs)
...
if __name__ == "__main__":
    ...
    if is_saved_model:
        tokenizer, model = get_model_tokenizer(
            base_model=base_model_name, trained_model_path=trained_model_path
        )
