...
def preprocess_function(
    examples, text_column, summary_column, tokenizer, padding, max_target_length
):
...
if __name__ == "__main__":
    ...
    tokenizer, model = get_model_tokenizer(model_name=model_name)

    dataset_name = "cnn_dailymail"
    data, text_column, summary_column, column_names = get_dataset(dataset_name=dataset_name)
    ...
    data_collator = DataCollatorForSeq2Seq(
        tokenizer, model=model, label_pad_token_id=training_args["label_pad_token_id"]
    )
    ...
    for epoch in range(training_args["train_epoch"]):
        ...
        gen_kwargs = {"max_length": val_max_target_length, "num_beams": 4}
        if epoch >= 0:
            to_dump = []
            ...
            for step, batch in enumerate(eval_dataloader):
                with torch.no_grad():
                    ...
                    decoded_source = tokenizer.batch_decode(
            ...
            with open(
                training_args["output_dir"] + "prediction_epoch{}.json".format(epoch), "w"
            ) as f:
            ...
            print(
                f"At epoch {epoch}, the evaluation result is {result}, the best score is {best_val_score} at epoch {best_epoch}"
            )
        ...
        unwrapped_model.save_pretrained(
            training_args["output_dir"] + f"/{epoch}", save_function=accelerator.save
        )
