...
def generate_vectorized_phrases(technical_phrase, wise_phrase):
    ...
    padded_technical_seq = nn.functional.pad(
        technical_seq, (0, sequence_length - len(technical_seq))
    )
...
def generate_random_emb_visualization(random_embedding, sequence_length, final_output_len, title):
    ...
    for i in range(2):
        ...
        matrix = (
            random_embedding[i, :, :].reshape(sequence_length, final_output_len).detach().numpy()
        )
...
def generate_fixed_emb_visualization(fixed_embedding, sequence_length, final_output_len, title):
    ...
    for i in range(2):
        ax = plt.subplot(1, 2, 1 + i)
        matrix = (
            fixed_embedding[i, :, :].reshape(sequence_length, final_output_len).detach().numpy()
        )
        cax = ax.matshow(matrix)
        plt.gcf().colorbar(cax)
        plt.title(title[i], y=1.2)
...
if __name__ == "__main__":
    ...
    random_weights_embedding_layer = PositionEmbeddingLayer(
        sequence_length, total_vocabulary, final_output_len
    )
    ...
    random_embedding = random_weights_embedding_layer(vectorized_phrases)
    fixed_embedding = fixed_weights_embedding_layer(vectorized_phrases)
    generate_random_emb_visualization(
        random_embedding, sequence_length, final_output_len, ["Tech Phrase", "Wise Phrase"]
    )
    generate_fixed_emb_visualization(
        fixed_embedding, sequence_length, final_output_len, ["Tech Phrase", "Wise Phrase"]
    )
