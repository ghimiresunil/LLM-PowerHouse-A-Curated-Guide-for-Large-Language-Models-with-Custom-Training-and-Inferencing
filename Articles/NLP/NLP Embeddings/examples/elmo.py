...
def plot_embeddings(y: np.ndarray, sentences: List[str]) -> go.Figure:
    ...
    fig.update_layout(
        width=900, height=600, title_text="Elmo Embeddings represented in 2 dimensions"
    )
    return fig


def find_similar_sentences(
    search_string: str, embeddings: np.ndarray, sentences: List[str], results_returned: int
    ...
    embeddings2 = hub.load(ELMO_URL).signatures["default"](
        tf.constant(
            [search_string],
        )
    )["default"]
    ...
    for i, j in cosine_similarities.nlargest(int(results_returned)).iteritems():
        similar_score = j
        similar_sentence = " ".join(
            [word if word.lower() in search_string else word for word in sentences[i].split()]
        )
...
if __name__ == "__main__":
    ...
    similar_scores, similar_terms = find_similar_sentences(
        SEARCH_STRING, embeddings, sentences, RESULTS_RETURNED
    )

    # Create a DataFrame from similarity scores and similar terms
    similarity_df = pd.DataFrame(
        {"Similarity Score": similar_scores, "Similar Terms": similar_terms}
    )
    print(similarity_df)
