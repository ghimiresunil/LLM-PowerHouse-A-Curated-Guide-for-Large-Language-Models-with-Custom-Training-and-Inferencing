...
def main(fname, window_size, epochs):
    ...
    X_train, y_train = generate_training_data(
        window_size=window_size, sentence_token=sentence_token, vocab_index=vocab_index
    )

    w2v = CBOWWord2Vec(words=data, word_index=vocab_index)

    w2v.train(epochs=epochs, x_train=X_train, y_train=y_train)

    while True:
        embedding_word = input(
            "Please Enter the word you are looking to see the skipgram prediction"
        )
...
def parser_args():
    ...
    parser.add_argument("--epochs", type=int, default=10, help="Enter epochs to train the model.")
