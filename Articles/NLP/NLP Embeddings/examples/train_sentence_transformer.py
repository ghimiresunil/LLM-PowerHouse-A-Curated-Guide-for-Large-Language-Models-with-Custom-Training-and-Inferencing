...
def parse_args():
    ...
    parser.add_argument("--epochs", type=int, default=4, help="Number of epochs to train model on")
...
def train(dataset_path, model_name, output_dir, train_batch_size, num_epochs):
    ...
    def samples_from_dataset(dataset):
        samples = [
            InputExample(texts=[e["sentence1"], e["sentence2"]], label=e["similarity_score"] / 5)
            for e in dataset
        ]
        return samples
    ...
    initial_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
    ...
    test_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
        test_samples, name="stsb-multi-mt-test"
    )
...
if __name__ == "__main__":
    ...
    train(
        args.dataset_path, args.model_name, args.output_dir, args.train_batch_size, args.num_epochs
    )
